...
for i, scene in enumerate(scenes_data):
    ...
    # 1. TTS with explicit path cleanup
    ...
    try:
        ...
        if scene_voiceover.duration > 0.5: scene_voiceover = scene_voiceover.subclip(0.1)
    except: scene_voiceover = None
    ...
    video_url = None
    try:
        res = requests.get(f"https://api.pexels.com/videos/search?query={keyword}&per_page=15&orientation=landscape", headers=headers, timeout=10).json()
        if res.get('videos'):
            for v in res['videos']:
        ...
        if not video_url: video_url = "https://player.vimeo.com/external/372064106.sd.mp4?s=d4052309831778170d62"
        
        vid_path = f"vid_{i}.mp4"
        with open(vid_path, "wb") as f: f.write(requests.get(video_url, timeout=30).content)
...
for i, c in enumerate(loaded_clips):
    if i > 0: loaded_clips[i] = c.crossfadein(CROSSFADE_DUR)
...
# FIX: Explicit fps and fps conversion applied to avoid errors
